var Unit = function () {
                ...
                events: {
                    'click .OpenEditItem': function (e, value, row, index) {
                        Sv.ChecPermission("View", function () {
                            var url = "/Admin/BizAccount/ShowModal";
                            var model = {
                                id: row.Id,
                                action: "Edit",
                                status: row.Status
                            ...
                            Sv.BindPopup(url, model, function (rs) {
                                ...
                                $('#formDetail #txtCreditAccount').val(row.CreditAccountCode).select2();
                                $('#formDetail #txtDebtAccount').val(row.DebtAccountCode).select2();
                                base.OpentDisable();
                            });
                        });
                    }
    ...
    this.SubmitServer = function (action, id) {
        var $form = $("#formDetail").on();
        var url = "/BizAccount/Create";
        if (action === "Edit") {
            url = "/BizAccount/Update";
        }
        if ($form.valid(true)) {
            Sv.AjaxPost({
                Url: url,
