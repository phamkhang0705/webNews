var Unit = function () {
    ...
    this.Columns = function () {
        var obj = [
         ...
         Sv.BootstrapTableColumn("Number", {
                title: 'ĐM tồn nhỏ nhất',
                field: 'WarningMin',
                width: "120px",
                align: "right"
            }),
            Sv.BootstrapTableColumn("Number", {
                title: 'ĐM tồn lớn nhất',
                field: 'WarningMax',
                width: "120px",
                align: "right"
            }),
            ...
            Sv.BootstrapTableColumn("String", {
                ...
                events: {
                    'click .OpenEditItemStorageCard': function (e, value, row) {
                        Sv.ChecPermission("View", function () {
                            var url = "/Admin/ProductManagement/ShowModalStorageCard";
                            productCode = row.ProductCode;
                            var model = {
                                productCode: row.ProductCode, action: "View"
                            };
                    ...
                    'click .OpenEditItemInventory': function (e, value, row) {
                        Sv.ChecPermission("View", function () {
                            var url = "/Admin/ProductManagement/ShowModalInventory";
                            productCode = row.ProductCode;
                            var model = {
                                productCode: row.ProductCode, action: "View"
                            };
                    ...
                    'click .OpenEditItemDetail': function (e, value, row) {
                        Sv.ChecPermission("View", function () {
                            var url = "/Admin/ProductManagement/ShowModalDetail";
                            var model = {
                                productCode: row.ProductCode, action: "View"
                            };
                            Sv.BindPopup(url, model, function (rs) {
                    ...
                    'click .OpenEditItemDelete': function (e, value, row) {
                        Sv.ChecPermission("Delete", function () {
                            Dialog.ConfirmCustom("Xác nhận", 'Bạn có muốn ngừng kinh doanh hàng hóa ' + (row.ProductName == null ? 'này' : row.ProductName) + '?', function () {
    ...
    this.SubmitServer = function (action, id) {
        Sv.Loading();
        var url = "/ProductManagement/Delete";
        Sv.AjaxPost({
            Url: url,
            Data: {productId:id}
        },
