var Unit = function () {
        ...
        var obj = [
            ...
            , Sv.BootstrapTableColumn("string", {
                ...
                formatter: function (value) {
                    if (value == 0) { return "Không hoạt động" }
                    if (value == 1) { return "Hoạt động" }
                    if (value == 2) { return "Xóa" }
                }
            }), Sv.BootstrapTableColumn("String", {
                title: "Thao tác",
                align: "Center",
                ...
                events: {
                    'click .OpenEditItem': function (e, value, row, index) {
                        Sv.ChecPermission("View", function () {
                            var url = "/Admin/SubjectDetail/ShowModal";
                            var model = {
    ...
    this.SubmitServer = function (action, id) {
        ...
        var url = "/SubjectDetail/Create";
        if (action == "Edit") {
            url = "/SubjectDetail/Update";
        }
        if ($form.valid(true)) {
            Sv.AjaxPost({
                Url: url,
    ...
    this.GetFormSearchData = function () {
        var obj = {};
        obj.SubjectId = $('#txtSubjectId').val();
...
$(document).ready(function () {
    ...
    unit.$table.bootstrapTable(Sv.BootstrapTableOption({
        url: "/Admin/SubjectDetail/GetData",
    ...
    unit.$btnSearchSubmit.click(function () {
        ...
        $('#txtSubjectId').val('-1').trigger('change');
    });
