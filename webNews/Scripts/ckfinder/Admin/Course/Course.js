var Unit = function () {
        ...
        var obj = [
            ...
            Sv.BootstrapTableColumn("string", {
                ...
                formatter: function (value, data, index) {
                    if (data.Type == 1) {
                        return "Khóa học ghép";
                    } else {
                        return "Khóa học riêng";
                    }
                }
            }),
             Sv.BootstrapTableColumn("string", {
                ...
                events: {
                    'click .OpenEditItem': function (e, value, row, index) {
                        Sv.ChecPermission("View", function () {
                            var url = "/Admin/CourseManagerment/ShowModal";
                            var model = {
    ...
    this.LoadTableSearch = function () {
        base.$table.bootstrapTable('refreshOptions', {
            url: "/Admin/CourseManagerment/GetData",
    ...
    this.SubmitServer = function (action, id) {
        ...
        else
        {
            ...
            var url = "/CourseManagerment/Create";
            if (action == "Edit") {
                url = "/CourseManagerment/Update";
            }
            if ($form.valid(true)) {
                Sv.AjaxPost({
                    Url: url,
    ...
    this.GetFormSearchData = function () {
        var obj = {};
        obj.Code = $('#txtCourseCode').val();
        obj.Name = $('#txtCourseName').val();
        obj.KindOfCourseId = $('#ddlKindOfCourse :selected').val();
        obj.Active = $("#ddlActive :selected").val();
        obj.Type = $("#ddlType :selected").val();
        return obj;
    }
